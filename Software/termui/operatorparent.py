""" Control Program - Operator Parent
    ...
    They get commands or requests from interfaces; through
    through the operator's comqueue and send responses through
    the interfaces respqueue called answerqueue in the operator.
    Also operators send communications to agents; through the
    agent's comqueue and get responses from the operator's 
    respqueue. An operator can send a command to an agent
    triggered by an incoming command or by itself (internally
    triggered).
...
class OperatorParent(InterParent, AgentParent):
    ...
    def __init__(self, config, name = ''):
        ...
        """
        self.name = name
        self.comqueue = queue.Queue() # Queue object for queries from interfaces
        self.respqueue = queue.Queue() # Queue objects for responses from agents
    ...
    def __call__(self):
        ...
        while not self.exit:
            # Look for task
            task, answerqueue = self.comqueue.get()
            ...
            # If Echo agent exists, send task to it
                ...
                answerqueue.put(respfull.strip())
